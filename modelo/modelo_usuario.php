<?php
    class Modelo_Usuario{
        ...
        function listar_clasificacion_combo(){
            $sql = "call SP_LISTAR_COMBO_CLASIFICACION()";
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
				while ($consulta_VU = mysqli_fetch_array($consulta)) {
                    $arreglo[]=$consulta_VU;
        ...
        function Modificar_Datos_Usuario($idusuario,$sexo,$clasifica,$catego,$numero,$observaciones,$rol,$email,$nombre){
            $sql = "call SP_MODIFICAR_DATOS_USUARIO('$idusuario','$sexo','$clasifica','$catego','$numero','$observaciones','$rol','$email','$nombre')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
				return 1;				
			}else{
				return 0;
        ...
        function Registar_Usuario($usuario,$contra,$sexo,$clasifica,$catego,$numero,$observa,$rol,$email,$ruta,$nombre){
            $sql = "call SP_REGISTRAR_USUARIO('$usuario','$contra','$sexo','$clasifica','$catego','$numero','$observa','$rol','$email','$ruta','$nombre')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
